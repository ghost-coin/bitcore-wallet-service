...
var Bitcore_ = {
  ...
  part: require('ghost-bitcore-lib'),
};
...
WalletService.prototype._getActiveAddresses = function(cb) {
  ...
  self.storage.fetchAddressesWithBalance(self.walletId, function(err, addressesWB) {
    ...
    self.storage.fetchNewAddresses(self.walletId, fromTs,  function(err, recent) {
      ...
      var result = _.uniqBy(_.union(addressesWB,  recent), 'address');
  ...
  function getChangeAddress(wallet, cb) {
    if (wallet.singleAddress) {
      ...
      if (opts.changeAddress) {
        ...
        return cb(null, wallet.createAddress(true, !!opts.coldStakingAddress));
      }
    }
  };
  ...
  function decorate(wallet, txs, addresses, proposals, notes) {
    ...
    return _.map(txs, function(tx) {
      ...
      if (tx.outputs.length || tx.inputs.length) {
        ...
        if (tx.isCoinStake) {
          action = 'staked';
          // TX Fees should always be negative on a staked tx, but lets just do a check to make sure
          if (tx.fees < 0) {
            amount = tx.fees * -1
          } else {
            amount = 0;
...
WalletService.prototype.scan = function(opts, cb) {
  ...
  function scanBranch(wallet, derivator, cb) {
    var inactiveIndexCounter = 0;
    var allAddresses = [];
    var gap = Defaults.SCAN_ADDRESS_GAP;
    var addresses = [];
    var activeDerivativeIndex = false;
    var numAddrsPerIndex;

    async.whilst(function() {
      self.logi('Scanning addr branch: %s index: %d gap %d ', derivator.id, derivator.index(), inactiveIndexCounter);
      return inactiveIndexCounter < gap;
    }, function(next) {
      if (_.isEmpty(addresses)) {
        var addr = derivator.derive();
        ...
        self.storage.storeWallet(wallet, function(err) {
          ...
          _.each([false, true], function(isChange) {
            derivators.push({
              ...
              derive: _.bind(wallet.createAddresses, wallet, isChange),
            ...
            self.storage.fetchAddressIndexCache(wallet.id, derivator.id, function(err, index){
              ...
              if (diff>0) {
                ...
                for(var i =0; i<diff; i++) {
                  var addrs = derivator.derive();
                  if (_.isArray(addrs)) {
                    _.each(addrs, function(a) { addresses.push(a); });
                  } else {
 ...
 */
WalletService.prototype.updateColdStakingSetup = function(opts, cb) {
  var self = this;
 ...
 */
WalletService.prototype.getColdStakingSetup = function(cb) {
  var self = this;
 ...
 */
WalletService.prototype.getColdStakingAddresses = function(cb) {
  var self = this;
    ...
    self.getWallet({}, function(err, wallet) {
      ...
      if (wallet.singleAddress) {
        self.storage.fetchAddresses(self.walletId, function(err, addrs) {
          ...
          const spendAddress = _.head(addrs);
